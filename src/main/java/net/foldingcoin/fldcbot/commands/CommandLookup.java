    ...
    public void processCommand (BotBase bot, IChannel channel, IMessage message, String[] params) {
        ...
        if (params.length >= 1) {
            ...
            if (users.isEmpty()) {
                ...
                double newFLDC = (double) (user.getNewCredit() + 0f) / FLDCStats.getNewPoints() * ((DistributionUtils.getDaysSinceLastDistribution() + 1) * 250000);
                //On the first day of the month, there are no new points
                if (FLDCStats.getNewPoints() == 0) {
                    newFLDC = 0;
                }
                embed.appendField("Unpaid FLDC", String.format("%.8f", newFLDC) + "", true);
